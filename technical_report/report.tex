...
% Title and Author
\title{\textbf{LifeLink: Resilient Mesh Communication in Contested Environments \\ via Semantic Compression and Economic Asymmetry}}
...
\author{
  ...
  \texttt{rninamdar@wpi.edu} \\
  \texttt{vdaita@stanford.edu} \\
}
...
\subsection{Hardware Integration \& Miniaturization}
...
To address this, we are working on designing a prototype monolithic Custom PCB (Current state in Figure \ref{fig:pcb_design}) that integrates the ESP32-S3 SoC, the SX1262 LoRa transceiver, and a dedicated MPPT solar management circuit onto a single board.
This integration allows us to:
\begin{enumerate}
    \item \textbf{Reduce Footprint:} Compressing the node volume by $\approx$60\%, allowing for inconspicuous deployment in urban debris.
    \item \textbf{Optimize RF Path:} Implementing a custom impedance-matched trace antenna to reduce signal loss compared to SMA connectors.
    \item \textbf{Slash Costs:} Consolidating the Bill of Materials (BOM) drives the unit cost from \$40 (prototype) toward our target of \$5 at volume.
\end{enumerate}

\begin{figure}[h]
    \centering
    % Left Image: Schematic
    \begin{minipage}{0.48\columnwidth}
        \centering
        \includegraphics[width=\linewidth]{schematic.png} % REPLACE THIS
        \caption{Schematic Design}
    \end{minipage}\hfill
    % Right Image: PCB
    \begin{minipage}{0.48\columnwidth}
        \centering
        \includegraphics[width=\linewidth]{3D.png} % REPLACE THIS
        \caption{PCB Layout}
    \end{minipage}
    \caption{\textbf{Proposed Hardware Integration.} Moving to a single-board design reduces manufacturing cost and improves physical resilience.}
...
\subsection{Multilingual Model Generalization}
The current semantic compression model is trained on English crisis datasets. To support global humanitarian deployment, we plan to utilize \textbf{Federated Learning}. This would allow the global model to be fine-tuned on user devices for local languages and slang without transmitting raw text back to a central server, preserving privacy while adapting to regional dialects.
